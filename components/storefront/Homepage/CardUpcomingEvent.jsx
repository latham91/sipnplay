...
export default function CardUpcomingEvent({ event, long }) {
  return (
    <div className="relative">
      <div className="relative z-10 flex flex-col gap-4 p-4 border group hover:-rotate-3 my-transition bg-stone-50 backdrop-blur-sm">
        ...
        <div className="flex flex-col gap-4 p-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm">
              🕒 {event.startTime}{" "}
              {event.startTime.slice(0, 2) >= 12 ? "PM" : "AM"} -{" "}
              {event.endTime} {event.endTime.slice(0, 2) >= 12 ? "PM" : "AM"}
            </p>
          </div>
          <p
            className={`text-stone-400 text-sm ${
              long ? "line-clamp-3" : "line-clamp-3"
            }`}
          >
            {event.description}
          </p>
